...
> **Nota de evolución (hotfix documental de la Fase 21, 23/09/2026).** El estado de arriba es el de la Fase 13 y se conserva. Desde entonces el servicio **sí existe** como experimento: la Fase 15 generó el dataset sintético, entrenó el modelo y creó el servicio FastAPI; la Fase 16 lo integró con Laravel y la Fase 17 lo validó en entorno de pruebas. Sigue siendo **experimental**, no autorizado para producción, y RF-29 sigue siendo candidato. El contrato implementado **difiere de la propuesta** de §3 y §7: la respuesta no tiene un campo de incertidumbre (`risk_score`, `risk_flag`, `threshold`, `model_version`, `freeze_fingerprint`, `status`) y la interfaz no la muestra; la tabla de §8 es la del 20/09/2026. El contrato vigente está en la skill `ml-risk-service` y en `ml-service/src/recruitment_ml/api/schemas.py`. Ver [`phase-15-closeout.md`](phase-15-closeout.md), [`phase-16-laravel-ml-integration.md`](phase-16-laravel-ml-integration.md) y [`phase-17-ml-validation.md`](phase-17-ml-validation.md).
